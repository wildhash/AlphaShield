...
class LenderAgent(BaseAgent):
    ...
    def __init__(self, db_client=None, embeddings_client=None, llm=None):
        super().__init__("Lender", db_client, embeddings_client, llm=llm)

    def originate_loan(
        self, borrower_id: str, principal: float, interest_rate: float = 8.0, term_months: int = 36
    ) -> str:
    ...
    def process(self, loan_id: str, **kwargs) -> dict[str, Any]:
        ...
        investment_returns = sum(
            t.get("amount", 0) for t in transactions if t.get("type") == "investment_return"
        )
        ...
        metrics = {
            ...
            "performance_ratio": (
                investment_returns / loan.split.investment_amount
                if loan.split.investment_amount > 0
                else 0
            ),
        }
    ...
    def assess_risk(self, loan_id: str) -> dict[str, Any]:
        ...
        poor_performance = [
            c for c in trading_contexts if c.get("data", {}).get("performance", 0) < 0
        ]
        if poor_performance:
            risk_score += 0.15
