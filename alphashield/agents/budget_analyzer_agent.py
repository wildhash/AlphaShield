...
class BudgetAnalyzerAgent(BaseAgent):
    ...
    def __init__(self, db_client=None, embeddings_client=None, llm=None):
        super().__init__("BudgetAnalyzer", db_client, embeddings_client, llm=llm)

    def analyze_budget(
        self, borrower_id: str, income: float, expenses: dict[str, float]
    ) -> dict[str, Any]:
        ...
        if warnings:
            self.log_action(
                "budget_warning",
                {"borrower_id": borrower_id, "budget_health": budget_health, "warnings": warnings},
            )

        return analysis

    def _generate_budget_recommendations(
        self, income: float, expenses: dict[str, float], actual_needs: float, actual_wants: float
    ) -> list[str]:
        ...
        if actual_wants > recommended_wants:
            ...
            recommendations.append(f"Reduce discretionary spending by ${overage:.2f} per month")
    ...
    def forecast_budget(self, borrower_id: str, months: int = 12) -> dict[str, Any]:
        ...
        contexts = self.get_shared_context(
            agent_name=self.name, context_type="budget_analysis", limit=6
        )
        ...
        if contexts:
            ...
            forecast = {
                ...
                "recommendation": (
                    "Monitor monthly" if expense_ratio < 0.80 else "Intervention needed"
                ),
            }
        else:
            forecast = {"borrower_id": borrower_id, "message": "Insufficient data for forecast"}
