"""Base agent protocol and classes for AlphaShield multi-agent system."""
...
# Generic type variables for Agent protocol
T_in = TypeVar("T_in")
T_out = TypeVar("T_out")


class Agent(Protocol, Generic[T_in, T_out]):
    """Protocol for agents in the orchestration system.

    All agents should implement this interface to be compatible with
    the orchestrator graph.
    """

    def run(self, input: T_in, ctx: ContextPacket) -> T_out:
        """Execute agent logic with input and context.

        Args:
            input: Agent-specific input data
            ctx: Context packet with shared state
...
class BaseAgent(ABC):
    ...
    @abstractmethod
    def run(self, input: Any, ctx: ContextPacket) -> Any:
        """Execute agent logic with input and context."""
