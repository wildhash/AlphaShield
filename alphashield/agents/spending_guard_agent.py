...
class SpendingGuardAgent(BaseAgent):
    ...
    def __init__(self, db_client=None, embeddings_client=None, llm=None):
        super().__init__("SpendingGuard", db_client, embeddings_client, llm=llm)

    def analyze_spending(
        self, borrower_id: str, transactions: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        # Use median + 3×MAD instead of mean + 2σ so a single extreme value
        # cannot inflate the threshold above itself.
        median_amount = statistics.median(amounts)
        mad = statistics.median([abs(x - median_amount) for x in amounts])
        if mad == 0:
            mad = statistics.stdev(amounts) if len(amounts) > 1 else 0
        threshold = median_amount + 3 * mad if mad else float("inf")
    ...
    def process(self, loan_id: str, **kwargs) -> dict[str, Any]:
        ...
        recent_transactions = [t for t in transactions if (now - t.get("timestamp", now)).days <= 7]
