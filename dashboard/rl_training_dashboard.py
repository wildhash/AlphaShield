...
def load_training_runs(days: int = 30) -> pd.DataFrame:
    ...
    runs = list(collection.find({"started_at": {"$gte": cutoff}}).sort("started_at", -1))
...
def load_replay_buffer_stats() -> dict[str, int]:
    ...
    pipeline = [
        {"$group": {"_id": "$agent", "count": {"$sum": 1}, "avg_reward": {"$avg": "$reward"}}}
    ]

    results = list(collection.aggregate(pipeline))

    return {
        item["_id"]: {"samples": item["count"], "avg_reward": item.get("avg_reward", 0)}
        for item in results
    }
...
def main():
    ...
    with col2:
        successful = (
            len(training_runs[training_runs["status"] == "success"])
    ...
    with col3:
        deployed = len(training_runs[training_runs["deployed"]]) if not training_runs.empty else 0
            ...
            policy_data.append(
                {
                    ...
                    "Last Updated": (
                        policy_info["created_at"].strftime("%Y-%m-%d %H:%M")
    ...
    if not training_runs.empty:
        ...
        fig_improvement.add_hline(
            y=5.0, line_dash="dash", line_color="green", annotation_text="Deployment Threshold"
        )
    ...
    if replay_stats:
        ...
        fig_buffer.add_hline(
            y=100, line_dash="dash", line_color="red", annotation_text="Minimum Threshold"
        )
