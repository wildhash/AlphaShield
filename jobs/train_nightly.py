...
class NightlyTrainer:
    ...
    def run(self, agents: list[str] | None = None) -> dict[str, dict]:
        ...
        logger.info("=" * 60)
        logger.info("Starting nightly training pipeline")
        ...
        for agent_name in agents:
            try:
                ...
                self.results[agent_name] = {"status": "error", "error": str(e)}
    ...
    def _train_agent(self, agent_name: str) -> dict:
        ...
        if not replay_data or len(replay_data) < 100:
            logger.warning(
                f"Insufficient data for {agent_name}: {len(replay_data) if replay_data else 0} samples"
            )
            return {
                "status": "skipped",
        ...
        if improvement >= self.deployment_threshold:
            if not self.dry_run:
                ...
                logger.info("🔸 Would deploy new policy (dry run)")
                deployed = False
        else:
            logger.info(
                f"⏭️ Skipping deployment: improvement {improvement * 100:.2f}% < threshold {self.deployment_threshold * 100:.1f}%"
            )
    ...
    def _load_replay_data(self, agent_name: str) -> list[dict]:
        ...
        try:
            collection = (
                self.db.get_collection("replay_buffer")
            ...
            cursor = collection.find(
                {"agent": agent_name, "timestamp": {"$gte": cutoff_date}}
            ).sort("timestamp", 1)
    ...
    def _deploy_policy(self, agent_name: str, bandit: LinUCB, improvement: float) -> Policy:
        ...
        return self.policy_manager.bump_version(
            agent=agent_name, algo="LinUCB", params=params, metadata=metadata
        )
    ...
    def _log_summary(self, start_time: datetime):
        ...
        for agent, result in self.results.items():
            ...
            if status == "success":
                ...
                logger.info(
                    f"{deployed} {agent}: improvement={result.get('improvement', 0) * 100:.2f}% v{result.get('current_version', 0)}→v{result.get('new_version', 0)}"
                )
            elif status == "skipped":
...
def main():
    ...
    trainer = NightlyTrainer(
        window_days=args.window_days, deployment_threshold=args.threshold, dry_run=args.dry_run
    )
