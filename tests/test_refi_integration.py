...
class TestRefiHappyPath(unittest.TestCase):
    ...
    def test_refi_happy_path(self):
        ...
        self.assertGreaterEqual(
            coverage_ratio, 1.30, f"Coverage ratio {coverage_ratio} below minimum 1.30"
        )

        # Verify allocation constraints
        allocation = bundle.coverage.get("allocation", {})
        for asset, weight in allocation.items():
            self.assertLessEqual(weight, 0.40, f"{asset} weight {weight} exceeds max 0.40")
            self.assertGreaterEqual(weight, 0.0, f"{asset} weight {weight} is negative")

        # Verify allocation sums to ~1.0
        total_weight = sum(allocation.values())
        self.assertAlmostEqual(
            total_weight, 1.0, places=1, msg=f"Total allocation {total_weight} not close to 1.0"
        )
        ...
        self.assertTrue(
            required_nodes.issubset(node_names), f"Missing nodes: {required_nodes - node_names}"
        )

        # Verify all events succeeded
        for event in bundle.audit_trail:
            self.assertEqual(event["status"], "success", f"Node {event['node']} failed")

        # Verify fairness (interest rate reasonable)
        interest_rate = bundle.offer.get("interest_rate", 0)
        self.assertLessEqual(interest_rate, 12.0, f"Interest rate {interest_rate}% too high")
        self.assertGreaterEqual(
            interest_rate, 5.0, f"Interest rate {interest_rate}% suspiciously low"
        )
    ...
    def test_emergency_allocation(self):
        ...
        self.assertGreaterEqual(
            bonds_allocation, 0.05, f"Bonds allocation {bonds_allocation} below minimum 0.05"
        )
