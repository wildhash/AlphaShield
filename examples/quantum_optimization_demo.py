...
def main():
    ...
    for _i, (asset, weight) in enumerate(zip(asset_names, optimized_weights, strict=False)):
    ...
    for _i, (asset, weight) in enumerate(zip(asset_names, classical_weights, strict=False)):
        print(f"      {asset:6s}: {weight:.4f} ({weight*100:.2f}%)")
